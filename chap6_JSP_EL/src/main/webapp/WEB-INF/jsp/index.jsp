...
<body>
	...
	목적 : EL은 JSP 페이지에서 요청(request)이나 세션(session)과 같은 데이터를 쉽게 가져와서 출력하는 주로 사용
	문법 : $ { } 사용해서 값을 출력
	...
	문법 : prefix="설정한변수명" 설정한변수명:기능작성
		  prefix 안에 변수명을 설정할 때는 http://java.sun.com/jsp/jstl/core 위 주소에서 core로 끝날 때
		  마지막에 끝나는 단어 첫 글자를 따서 주로 많이 사용
		  c언어가 아님!
	단점 : 문법이 복자할 수 있음 태그를 익히는데 시간 소요
	</pre>	
	...
	value : 저장할 실제 값(value) 문자열 숫자 배열 객체 등 어떤 데이터 유형도 저장할 수 있음
	</pre>
	...
	// 데이터를 request 객체 저장
	request.setAttribute("username", name); // username 변수명 안에 홍길동 이라는 이름을 저장 저장된 이름을 다른데서 사용
	request.setAttribute("userage", age); // userage 변수명 안에 20이라는 숫자를 저장하고 다른데에서 숫자를 사용
	%>
	...
	<%
	request.setAttribute("msg", "안녕하세요! index.jsp에서 hello.jsp으로 데이터 전달해드립니다.");
	
	// hello.jsp 페이지로 전달
	// dispatcher : 전달자 상태를 전달한다는 의미 보내다 운항관리사 배차원 : dispatcher
	
	// servlet springboot를 사용하지 않을 때 사용하는 방법
	// spring boot 사용해서 controller로 넘겨줄 때?
